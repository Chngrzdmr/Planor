...
namespace Planor.Sayfalar
{
    ...
    {
        SistemForm sistm = new SistemForm();
        ...
        private void PanodanImageBTN_Click(object sender, EventArgs e)
        {
            ...
            {
                string kayityolu = Path.GetTempPath() + DateTime.Now.ToFileTime() + "_satis-sozlesmesi_img.png";
            ...
            else MessageBox.Show("Lütfen ilk önce \"Satış Sözleşmesi\" 'ni Sağtuş ile Kopyala işlemi yapınız...");
        }

        private void SatistanIptaliBaslatBTN_Click(object sender, EventArgs e)
        {
            if (txt_kayityolu.Text != "" &&
                cmb_sirket.Items[cmb_sirket.SelectedIndex].ToString() != "" &&
                txt_policeno.Text != "" &&
            ...
            else MessageBox.Show("Lütfen önce bilgileri eksiksiz doldurunuz...");
        ...
        {
            cmb_sirket.Items.Add("sirket_adi");
        }
        ...
        private void txtesas4_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                txts4.Text = String.Format("{0:N0}", Convert.ToInt32(txtesas4.Text.Replace(".", "")));
                txts0.Text = String.Format("{0:N0}", Math.Truncate(Convert.ToInt32(txtesas4.Text.Replace(".", "")) * 3.00));
                txts1.Text = String.Format("{0:N0}", Math.Truncate(Convert.ToInt32(txtesas4.Text.Replace(".", "")) * 2.35));
                txts2.Text = String.Format("{0:N0}", Math.Truncate(Convert.ToInt32(txtesas4.Text.Replace(".", "")) * 1.90));
                txts3.Text = String.Format("{0:N0}", Math.Truncate(Convert.ToInt32(txtesas4.Text.Replace(".", "")) * 1.45));
                txts5.Text = String.Format("{0:N0}", Math.Truncate(Convert.ToInt32(txtesas4.Text.Replace(".", "")) * 0.90));
                txts6.Text = String.Format("{0:N0}", Math.Truncate(Convert.ToInt32(txtesas4.Text.Replace(".", "")) * 0.78));
                txts7.Text = String.Format("{0:N0}", Math.Truncate(Convert.ToInt32(txtesas4.Text.Replace(".", "")) * 0.58));
                txts8.Text = String.Format("{0:N0}", Math.Truncate(Convert.ToInt32(txtesas4.Text.Replace(".", "")) * 0.50));

                txt1a4.Text = String.Format("{0:N0}", Math.Truncate(Convert.ToInt32(txtesas4.Text) * 1.0475));
                ...
                txty4.Text = String.Format("{0:N0}", Math.Truncate(Convert.ToInt32(txtesas4.Text) * 1.696));
            ...
            //if (!string.IsNullOrEmpty(txtesas4.Text))
            //{
            ...
            //    try
            //    {
            //        nValue = double.Parse(txtesas4.Text, System.Globalization.NumberStyles.AllowThousands, nfi);
            //    }
            //    catch (Exception se)
            //    {
            //        bError = true;
            //        MessageBox.Show("Error : " + se.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            //    }
            //    finally
            //    {
            //        if (!bError)
            //        {
            //            txtesas4.Text = string.Format(nfi, "{0:N0}", nValue);
            //            txtesas4.Text = txtesas4.Text.Replace(" ", "");
            //            txtesas4.Select(txtesas4.Text.Length, 0);
